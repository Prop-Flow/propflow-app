...
import { Loader2, TrendingDown, TrendingUp, DollarSign, Wallet, Plus, AlertTriangle, Building, FileText } from 'lucide-react';
...
export default function PropertyFinancialsPage() {
    ...
    }, [propertyId, fetchFinancials, fetchNetIncome, fetchValuation, fetchProjections, fetchTenants]);
    ...
    return (
        <DashboardShell role={currentRole}>
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4" onClick={() => setShowRentRoll(false)}>
                            <div className="bg-slate-900 w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-700 p-6" onClick={e => e.stopPropagation()}>
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <button onClick={() => setShowRentRoll(false)} className="text-slate-400 hover:text-white">✕</button>
                                </div>
                ...
                {/* Lease Builder Modal */}
                {showLeaseBuilder && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowLeaseBuilder(false)}>
                        <div className="w-full max-w-2xl" onClick={e => e.stopPropagation()}>
                            <LeaseBuilder
                                propertyId={propertyId}
                                tenants={tenants}
                                onComplete={() => {
                                    // Optional: Refresh things or simply close
                                    // setShowLeaseBuilder(false); // Let the user close manually from success screen if desired, or add auto-close logic in the component
                                }}
                            />
                            <div className="mt-4 text-center">
                                <button onClick={() => setShowLeaseBuilder(false)} className="text-sm text-slate-400 hover:text-white transition-colors">Close</button>
                            </div>
                        </div>
                    </div>
                )}
